/* ============================================================================
 * PROPOSAL ONLY — NOT APPLIED, NOT A MIGRATION YET.
 *
 * This file lives in docs/ and is named .sql DELIBERATELY so that
 * node-pg-migrate never sees it and never attempts to apply it. It is written
 * as node-pg-migrate JavaScript (matching every existing file in migrations/)
 * so that, on owner approval, it can be renamed verbatim to
 *
 *     migrations/<timestamp>_create-trading-accounts.js
 *
 * and applied with no rewriting.
 *
 * Companion to docs/SPEC-LIVE-VERTICAL-SLICE-0.1.md. Read that spec's §1, §2,
 * §4, §5 and §19 before reviewing this schema — every table and every
 ...
 * terminal state (see the comment on trade_intents.signed_tx_b64). Spec test
 * T47 asserts this mechanically against the live schema.
 * ==========================================================================*/
...
exports.up = (pgm) => {
  ...
  pgm.createTable("wallet_ownership_proofs", {
    ...
    // base64 Ed25519 signature over the nonce. Public material: it cannot be
    // replayed (the nonce is single-use) and the key cannot be derived from it.
  ...
  // signature or fill, and contracts.test.ts enforces it. Every provenance
  // column below is exactly such a field. The EXIT LOGIC is shared instead of
  // the type (paper-position.ts:evaluateExit, widened to a structural
  // ExitEvaluable) so ARIA can never hold two opinions about when to exit.
